...
function resolveMedia(folder, filename) {
  if (!filename || filename === "" || filename === "null" || filename === "undefined") {
    return "media_folder/branding/placeholder.png";
  }
  ...
  if (cleanName.includes("/")) {
    return `media_folder/${cleanName}`;
  }
  return `media_folder/${folder}/${cleanName}`;
}
...
function createUniversalCardHtml(item, folder, fallbackFolder = "projects") {
  ...
  const actionLink = item.Action_Link || item.Live_Demo_URL || item.Form_Link || "viber://chat?number=%2B959775775020";
